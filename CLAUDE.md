...
The PyInstaller spec is `my_game_hub.spec`. The Inno Setup script `installer.iss` can produce a full Windows installer from the dist folder, but is not part of the regular build workflow yet. Current app version is **1.2**.
...
- **`widgets.py`** — All UI. Key classes:
  ...
  - `GameCard` — fixed 130×130 `QFrame`. Star (☆/★) sits at the top-right. Green play overlay appears on hover. Clicking the card body launches the game; dragging reorders within the grid.
